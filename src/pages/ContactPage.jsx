...
const OffererForm = ({ type, setLoading }) => {
  ...
  const initForm = () => {
    setLoading(true);
    api.userGetContact({ type }).then((res) => {
      setLoading(false);
  ...
  return (
    <form onSubmit={form.onSubmit((values) => submitForm(values))}>
      ...
      <Group position="center">
        <Button size="md" type="submit" mt="sm" mb={32} loading={isSubmit}>
          儲存
        </Button>
      </Group>
    </form>
...
const EngineerForm = ({ type, setLoading }) => {
  ...
  const initForm = () => {
    ...
    api.userGetContact({ type }).then((res) => {
      setLoading(false);
      const { email, serviceType, languages, phone, facebook, line, desc } =
        res.data.message;
      form.setFieldValue("email", email);
      form.setFieldValue(
        "serviceType",
        serviceType === "remote" ? "remote" : "office"
      );
